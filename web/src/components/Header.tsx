  ...
  return (
    <header className="site-header sticky top-0 z-50 px-4">
      <nav className="page-wrap flex flex-nowrap items-center gap-3 py-4 sm:gap-4">
        {/*
          收缩优先级（空间不足时）：theme 最先缩（已是单 icon）→ Logo 次之 → 导航最后。
          Logo 容器允许收缩(min-w-0)，字标在 <480px 退回 42·R 给导航让路；
        ...
        */}
        <Link to="/" className="flex min-w-0 flex-shrink items-center gap-2.5 overflow-hidden">
          ...
          >
            ...
            {/* <480px 只显 R，≥480px 显完整 research（Logo 先于导航让路） */}
            <span className="min-[480px]:hidden">R</span>
            <span className="hidden min-[480px]:inline">Research</span>
          </span>
        </Link>
