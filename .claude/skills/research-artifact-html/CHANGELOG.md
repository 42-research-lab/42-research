...
- **v1.7 (2026-08-05)**: 外链一律新标签打开（用户反馈）——产物内所有外部 `<a href="http...">` 必须带 `target="_blank"`（并配 `rel="noopener noreferrer"`，已有 rel 的并入 noopener）；validate_artifact.py 新增检查 #8 把该约定固化为质量门。存量 6 个产物（3 课题 × 双语）146 处外链已批量修复。
