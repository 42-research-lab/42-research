...
**分级规则**：
- `primary`（一手）= 官方文档、官方定价页、原始基准数据、官方公告、**自有可复现实验数据**
  （前提：环境/命令/版本已记录，台账脱敏后随产物公开）。权重最高。
- **内部速报/旧报告也是待验证来源**：公开发布前逐条复核其中的外部声明——课题 02 即纠正了内部速报的
  两处失实（「GPT 固定三档分辨率」实为任意分辨率至 4K；单点牌价实有口径分歧应写区间）。
- 同一厂商多个页面口径冲突（如 Arena 名次 No.2 vs No.3）→ 两边都引、注明冲突、不采信为结论依据。
...
| 使用中发现 | 当场动作 |
|---|---|
| 流程步骤在实战中证明缺失 / 顺序不对 | 修流程本身，不绕过它 |
| 引用分级 / DoD 遇到未覆盖的新情况（如：自有实验数据算什么 tier） | 补规则 + 把这次实例写进对应段落 |
| 与 `research-artifact-html` / `publish-research-topic` 交接处出现「没人管」的空隙 | 补交接说明，两边 skill 同步改 |
| 对抗验证中出现可复用的新质疑角度 | 沉淀进「对抗验证」角度清单 |
| 文档与仓库实际不符（路径 / 范例 / 脚本变了） | 以仓库为准当场修——失实文档比没文档更危险 |
